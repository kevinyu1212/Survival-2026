import { createAuthManager } from 'auth-kit';
import { createBillingManager } from 'billing-kit';
import { createAIStreamManager } from 'ai-stream-core';
...
async function runPaidFeatureSimulation() {
  ...
  const auth = createAuthManager({ provider: 'supabase' });
  auth.setSession({
  ...
  const billing = createBillingManager({ provider: 'stripe', secretKey: 'sk_live_mock_secure_key' });
  ...
  if (session) {
    ...
    if (!proAccessCheck.granted) {
      ...
      const aiManager = createAIStreamManager(5);
